...
namespace Solarix\SyliusAuthorizeNetPlugin\StateMachine;
...
final class Resolver implements StateResolverInterface
{
  ...
  public function resolve(BaseOrderInterface $order): void
  {
    ...
    [$totalPayed] = $this->getPaymentTotalWithState(
      $order,
    ...
    switch ($targetTransition) {
      ...
      case OrderPaymentTransitions::TRANSITION_PARTIALLY_REFUND:
        [$totalRefunded] = $this->getPaymentTotalWithState(
          $order,
  ...
  private function getTargetTransition(OrderInterface $order): ?string
  {
    [
      ...
      $refundedPayments,
    ] = $this->getPaymentTotalWithState(
      $order,
      PaymentInterface::STATE_REFUNDED
    );

    if (
      0 < $refundedPayments->count() &&
      $refundedPaymentTotal >= $order->getTotal()
    ) {
      return OrderPaymentTransitions::TRANSITION_REFUND;
    }

    if (
      0 < $refundedPaymentTotal &&
      $refundedPaymentTotal < $order->getTotal()
    ) {
    ...
    if (
      (0 < $completedPayments->count() &&
        $completedPaymentTotal >= $order->getTotal()) ||
    ...
    if (
      0 < $authorizedPayments->count() &&
      $authorizedPaymentTotal >= $order->getTotal()
    ) {
      return OrderPaymentTransitions::TRANSITION_AUTHORIZE;
    }

    if (
      0 < $authorizedPaymentTotal &&
      $authorizedPaymentTotal < $order->getTotal()
    ) {
   ...
   */
  private function getPaymentsWithState(
    OrderInterface $order,
    string $state
  ): Collection {
    return $order
      ->getPayments()
      ->filter(function (PaymentInterface $payment) use ($state) {
   ...
   * @return array
   */
  private function getPaymentTotalWithState(
    OrderInterface $order,
    string $state
  ): array {
    $paymentTotal = 0;
